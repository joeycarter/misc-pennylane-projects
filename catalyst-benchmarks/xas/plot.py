...
def parse_benchmarks(benchmarks: list, args):
    ...
    for benchmark in benchmarks:
        ...
        num_steps = benchmark["params"]["num_steps"]
        n1 = benchmark["params"]["N1"]
        ncas = benchmark["params"]["ncas"]
        ...
        if num_steps not in df_dict:
            df_dict[num_steps] = {}

        key = f"{n1}-{ncas}"

        df_dict[num_steps][(key, mode, "mean")] = benchmark["stats"]["mean"]
        df_dict[num_steps][(key, mode, "stderr")] = benchmark["stats"][
            "stddev"
        ] / np.sqrt(benchmark["stats"]["rounds"])

    # https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.from_dict.html
    df = pd.DataFrame.from_dict(df_dict, orient="index")

    # Clean up dataframe
    df.index.name = "num_steps"
