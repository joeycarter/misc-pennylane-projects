...
def create_benchmark_fig_and_axes(figsize=[6.4, 6.4], title=None):
    """Create figure and axes for benchmark plot

    Returns the handle to the figure and the handles to a pair of axes. `ax1`
    is the handle for the upper panel where the benchmarking data is plotted and
    `ax2` is the handle to the lower panel where the ratios are plotted.

    The default figsize 6.4 x 6.4 is optimized for web and print publication.

    Global default plotting params are also set here, such as font size.
    """
    # Set plotting default params
...
def savefig_and_close(fig, fname_prefix, args):
...
def draw_benchmark_plot_legend(ax, where="left"):
    if where == "right":
        ax.legend(
            loc="upper right", frameon=False, bbox_to_anchor=(1.0, 1.0), fontsize=12
        )
    else:
        ax.legend(
            loc="upper left", frameon=False, bbox_to_anchor=(0.0, 0.94), fontsize=12
        )
